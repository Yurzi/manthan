...
class InterpolatingSolver:
    def __init__(self, first_part, second_part):
        ...
        self.solver.addFormula(
            Utils.miniSAT_clauses(first_part), Utils.miniSAT_clauses(second_part)
        )
    ...
    def interpolate(self, variable, shared_variables, assumptions=[], budget=0):
        return self.solver.getInterpolant(
            variable, Utils.miniSAT_literals(assumptions), shared_variables, budget
        )

    def getDefinition(
        self, input_variable_ids, output_variable_id, offset, compress=False
    ):
