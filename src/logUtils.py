...
class LogEntry:
    def __init__(self) -> None:
        ...
        # time
        self.total_time_start: float = -1.0
        self.total_time_end: float = -1.0
        self.preprocess_time_start: float = -1.0
        self.preprocess_time_end: float = -1.0
        self.datagen_time_start: float = -1.0
        self.datagen_time_end: float = -1.0
        self.leanskf_time_start: float = -1.0
        self.leanskf_time_end: float = -1.0
        self.refine_time_start: float = -1.0
        self.refine_time_end: float = -1.0
    ...
    def get_samples_acc(self):
        ...
        file_generate_cnf_cmd = (
            file_generate_cnf_exe
            + " "
            + leanskf_out_path
            + " "
            + cnf_file_path
    ...
    @staticmethod
    def from_file(filename) -> "LogEntry":
        f = pyzstd.open(filename, "rb")
        obj = pickle.load(f)
        f.close()
        return obj

    def before_save(self):
        # check all time flag is correct
        now_time = time.time()

        if self.preprocess_time_start < 0:
            self.preprocess_time_start = now_time

        if self.preprocess_time_end < 0:
            self.preprocess_time_end = now_time

        if self.datagen_time_start < 0:
            self.datagen_time_start = now_time

        if self.datagen_time_end < 0:
            self.datagen_time_end = now_time

        if self.leanskf_time_start < 0:
            self.leanskf_time_start = now_time

        if self.leanskf_time_end < 0:
            self.leanskf_time_end = now_time

        if self.refine_time_start < 0:
            self.refine_time_start = now_time

        if self.refine_time_end < 0:
            self.refine_time_end = now_time

        if self.total_time_start < 0:
            self.total_time_start = now_time

        if self.total_time_end < 0:
            self.total_time_end = now_time

        # check is sat
        if len(self.output_verilog) > 0:
            self.sat = True

    def to_file(self):
        self.before_save()
        mkdir("log")
        path = "log/" + str(self.instance_name) + ".pkl.zst"
        try:
            f = pyzstd.open(path, "wb")
            pickle.dump(self, f)
        except OSError:
            print("OSError: " + path)
        finally:
