...
class DefinabilityChecker:
    def __init__(self, formula, existentials):
        ...
        self.on_selector_dict = {v: v + (2 * self.max_variable) for v in existentials}
        on = [[v, -self.on_selector_dict[v]] for v in existentials]
        self.off_selector_dict = {v: v + (3 * self.max_variable) for v in existentials}
        off = []
        ...
        eq = [
            c
            for v in variables
            for c in Utils.equality(v, self.renaming[v], self.eq_selector_dict[v])
        ]
        self.max_variable = 5 * self.max_variable

        self.solver = InterpolatingSolver.InterpolatingSolver(
            formula + on, formula_copy + off + eq
        )
    ...
    def addClause(self, clause):
        ...
        renamed_clause = Utils.renameClause(clause, self.renaming)
    ...
    def checkDefinability(
        self, defining_variables, defined_variable, offset=None, assumptions=[]
    ):
        if offset is None:
        ...
        logging.debug(
            "Checking definability of {} in terms of {}.".format(
        ...
        assert (
            satisfiable != InterpolatingSolver.TRIBOOL_INDETERMINATE
        ), "SAT call indeterminate."

        if satisfiable == InterpolatingSolver.TRIBOOL_FALSE:
            return True, self.solver.getDefinition(
